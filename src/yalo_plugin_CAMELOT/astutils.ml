...
let rec skip_seq_let (e: OCAML_AST.expression) : OCAML_AST.expression =
  match e.pexp_desc with
...
let are_idents_same (el: OCAML_AST.expression) (er: OCAML_AST.expression) =
...
let is_pat_constr (pat: OCAML_AST.pattern) lident_name =
  match pat.ppat_desc with
  | Ppat_construct ({txt = Lident l; loc = _}, _) -> l = lident_name
  | _ -> false

let is_pat_tuple (pat: OCAML_AST.pattern) : bool =
  match pat.ppat_desc with
  | Ppat_tuple _ -> true
  | _ -> false

let is_pat_record (pat: OCAML_AST.pattern) : bool =
  match pat.ppat_desc with
  | Ppat_record _ -> true
  |  _ -> false

let is_case_constr (case: OCAML_AST.pattern) = is_pat_constr case

let is_case_const (case: OCAML_AST.pattern) =
  begin match case.ppat_desc with
    | Ppat_constant _ -> true
    | _ -> false
  end

let ident_of_let (pat: OCAML_AST.value_binding) : string =
...
let rec e_eq_any (exps: OCAML_AST.expression_desc list) : bool =
