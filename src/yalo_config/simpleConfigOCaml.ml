...
let parse_config_file str =
  ...
  and parse_list_semi end_kwd values =
    ...
    | Some (Kwd (",") ) when values != [] ->
       Stream.junk str;
       let values =
         match values with
         | head :: tail ->
            let head = parse_list_comma ";" [ head ] in
            head :: tail
         | _ -> assert false
       in
       parse_list_semi end_kwd values
    | _ ->
      let v = parse_option () in
      parse_list_semi end_kwd (v :: values)
  ...
  and parse_list_comma end_kwd values =
    ...
    | Some (Kwd kwd) when kwd = end_kwd ->
      Stream.junk str;
      List (List.rev values)
    | Some (Kwd "]") when end_kwd = ";" ->
      List (List.rev values)
    | _ ->
      let v = parse_option () in
      parse_list_comma end_kwd (v :: values)
...
let rec save_module with_help indent oc list =
  ...
  List.iter
    (fun (m, p) ->
       ...
       Printf.bprintf oc "%s}\n" indent)
    !subm
and save_list_comma indent oc list =
  match list with
    [] -> ()
  | [v] -> save_value indent oc v
  | v :: tail ->
      save_value indent oc v; Printf.bprintf oc ", "; save_list_comma indent oc tail
